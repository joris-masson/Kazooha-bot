...
class IdToTime(Extension):
    ...
    async def command(self, ctx: SlashContext, discord_id: str):
        log("SLASH", f"Commande slash `/id_to_time {discord_id}` utilisée par {ctx.author.username}({ctx.author.id}) dans #{ctx.channel.name}({ctx.channel.id}) sur {ctx.guild.name}({ctx.guild.id})")
